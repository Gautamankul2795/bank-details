...
'''class BanksView2(generics.RetrieveUpdateDestroyAPIView):
	"""
	...
	def get(self, request, bank_name, city):
		try:
			branch_qset = Banks.objects.filter(
				city__iexact=city, bank__name__icontains=bank_name)
...
class BanksView2(generics.ListCreateAPIView):
	...
	def get(self, request, bank_name, city):
		branch_qset = Banks.objects.filter(
		city__iexact=city, bank_name__icontains=bank_name)[:10]
		serializer = BanksSerializer(branch_qset, many=True)
		return Response(serializer.data)

class BanksViewSet(generics.RetrieveUpdateDestroyAPIView):
	"""
	GET api/ifsc/
	"""
